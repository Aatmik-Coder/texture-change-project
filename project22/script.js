document.getElementById('cont_col1').addEventListener('click', function() {
    location.href = 'n1/index.html'
}, false);

document.getElementById('cont_col2').addEventListener('click', function() {
    location.href = 'n2/index.html'
}, false);

document.getElementById('cont_col3').addEventListener('click', function() {
    location.href = 'n3/index.html'
}, false);
document.getElementById('cont_col4').addEventListener('click', function() {
    location.href = 'n5/index.html'
}, false);
document.getElementById('cont_col5').addEventListener('click', function() {
    location.href = 'n8/index.html'
}, false);

document.getElementById('cont_col6').addEventListener('click', function() {
    location.href = 'n9/index.html'
}, false);

document.getElementById('cont_col7').addEventListener('click', function() {
    location.href = 'n11/index.html'
}, false);
document.getElementById('cont_col8').addEventListener('click', function() {
    location.href = 'n12/index.html'
}, false);

